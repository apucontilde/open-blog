...
create table tenants (
    ...
    content_version bigint not null default 0,   -- bumped on any public-visible change (ETag anchor)
...
-- case-insensitive uniqueness; Go normalizes too
...
create table sessions (
    ...
    user_id    uuid not null references users(id) on delete cascade,
    scope      uuid,                       -- active tenant; NULL = none yet
...
create table posts (
    ...
    unique (tenant_id, id)         -- composite-FK anchor; blocks cross-tenant child attach
);
...
create table post_images (
    ...
    created_at timestamptz not null default now(),
    foreign key (tenant_id, post_id) references posts (tenant_id, id) on delete cascade,  -- blocks cross-tenant attach
    unique (tenant_id, r2_key)                                                            -- confirm idempotency
);
...
create table invitations (
    ...
    email      text,
    role       text not null check (role in ('owner','admin','editor','author')),
    token_hash bytea,                 -- role-baked link; NULL for pure-email invites
    expires_at timestamptz not null,
...
create table imports (
    ...
    created_at    timestamptz not null default now(),
    updated_at    timestamptz not null default now(),
    foreign key (tenant_id, post_id) references posts (tenant_id, id) on delete set null  -- blocks cross-tenant attach
);
...
create table jobs (
    id         uuid primary key default gen_random_uuid(),
    kind       text    not null,
    dedupe_key text,                 -- makes the ON CONFLICT idempotency enforceable
    payload    jsonb   not null default '{}'::jsonb,
    state      job_state not null default 'queued',
    attempts   integer not null default 0,   -- counts runs (incremented on claim), not failures
    run_after  timestamptz not null default now(),
    lease_until timestamptz,                  -- expire sweep requeues a stale running job
    last_err   text,
    created_at timestamptz not null default now(),
    updated_at timestamptz not null default now()
);
